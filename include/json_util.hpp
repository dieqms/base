...
public:
	...
	bool GetObject(size_t index, JSON_Object ** val) {
		return GetArrayObject(_array, index, val);
	}
	...
	static bool GetArrayObject(const JSON_Array *array, size_t index, JSON_Object ** val) {
...
public:
	...
	bool ParseString(const string & str) {
		_root_value = json_parse_string_with_comments(str.c_str());
		if (NULL == _root_value) {
			printf("[json_util] parse string %s fail!\n", str.c_str());
			return false;
		}
	...
	bool SaveStringPretty(std::string & str) {
		return SaveStringPretty(_root_value, str);
	}
	
	bool SaveString(std::string & str) {
		return SaveString(_root_value, str);
	}
	...
	static bool SaveStringPretty(JSON_Value * value, std::string & str) {
		bool ret = false;
		char * out = json_serialize_to_string_pretty(value);
	...
	static bool SaveString(JSON_Value * value, std::string & str) {
		bool ret = false;
		char * out = json_serialize_to_string(value);
